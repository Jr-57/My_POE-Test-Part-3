...
public class QuickChatMessaging {
    ...
    static ArrayList<ReceivedMessage> inbox = new ArrayList<>();
    static final String FILE_NAME = "messages.json";
    static final String INBOX_FILE = "inbox.json";
    ...
    public static void main(String[] args) {
        ...
        do {
            ...
            System.out.println("4. Simulate Receive Message");
    ...
    static void sendMessages(Scanner input) {
        ...
        for (int i = 0; i < num; i++) {
            ...
            while (true) {
                ...
                if (m.checkMessageLength().equals("Your Message is ready to send.")) {
                    break;
                } else {
                    System.out.println("Try again.\n");
                }
    ...
    static void checkInbox(Scanner input) {
        if (inbox.isEmpty()) {
        ...
        for (int i = 0; i < inbox.size(); i++) {
            ReceivedMessage m = inbox.get(i);
            if (!m.isRead) unreadCount++;
            System.out.println("\n" + (i + 1) + ". From: " + m.sender +
                             (m.isRead? "" : " [NEW]") +
                             " | " + m.timestamp.substring(0, 16));
        }
        System.out.println("\nUnread: " + unreadCount + "/" + inbox.size());

        System.out.print("\nEnter message number to read, or 0 to go back: ");
        int choice = input.nextInt();
        input.nextLine();

        if (choice > 0 && choice <= inbox.size()) {
            ReceivedMessage selected = inbox.get(choice - 1);
            selected.markAsRead();
            System.out.println("\n" + selected.printMessage());
        }
    }

    static void simulateReceive(Scanner input) {
        System.out.print("Enter sender number with + : ");
        String sender = input.nextLine();
        System.out.print("Enter message received: ");
        String msg = input.nextLine();

        ReceivedMessage received = new ReceivedMessage(sender, msg);
        inbox.add(received);
        System.out.println("New message received from " + sender);
    }
    ...
    static void saveInbox() {
        try {
            ...
            for (int i = 0; i < inbox.size(); i++) {
                ...
                json.append("{");
                json.append("\"messageID\":\"").append(m.messageID).append("\",");
                json.append("\"sender\":\"").append(m.sender).append("\",");
                json.append("\"message\":\"").append(m.message.replace("\"", "\\\"")).append("\",");
                json.append("\"timestamp\":\"").append(m.timestamp).append("\",");
                json.append("\"isRead\":").append(m.isRead);
                json.append("}");
                if (i < inbox.size() - 1) json.append(",");
            }
            json.append("]");
            Files.write(Paths.get(INBOX_FILE), json.toString().getBytes());
            System.out.println("Inbox saved to " + INBOX_FILE);
        } catch (Exception e) {
            System.out.println("Error saving inbox: " + e.getMessage());
        }
    }
    ...
    static void loadInbox() {
        try {
            if (!Files.exists(Paths.get(INBOX_FILE))) return;

            String content = new String(Files.readAllBytes(Paths.get(INBOX_FILE)));
            content = content.trim();
            if (content.equals("[]")) return;
            ...
            for (String obj : objects) {
                ...
                String id = parts[0].split(":\"")[1].replace("\"", "");
                String sender = parts[1].split(":\"")[1].replace("\"", "");
                String message = parts[2].split(":\"")[1].replace("\"", "").replace("\\\"", "\"");
                String timestamp = parts[3].split(":\"")[1].replace("\"", "");
                boolean isRead = parts[4].split(":")[1].equals("true");

                ReceivedMessage m = new ReceivedMessage(id, sender, message, timestamp, isRead);
                inbox.add(m);
            }
            System.out.println("Loaded " + inbox.size() + " messages from " + INBOX_FILE);
        } catch (Exception e) {
            System.out.println("No saved inbox found.");
        }
    }
}
...
class ReceivedMessage {
    ...
    public ReceivedMessage(String id, String sender, String message, String timestamp, boolean isRead) {
        this.messageID = id;
        this.sender = sender;
        this.message = message;
    ...
    String generateMessageID() {
        ...
        return "R" + String.valueOf(id); // R prefix for received
    }
